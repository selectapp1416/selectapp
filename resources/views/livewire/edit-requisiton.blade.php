<div>
    <form class="form" wire:submit.prevent="updateRequisition">
        ...
        @enderror
        <article class="form-group">
            <label for="">Area Asociada <span class="ast">*</span></label>
            <select name="work_area_id" id="charge_id" class="form-control" wire:model.live="work_area_id">
                <option value="{{$this->requisiton->work_area_id}}">{{$this->requisiton->work_area->work_area}}</option>
                @foreach($work_areas as $work_area)
                @if($work_area->id == $this->requisiton->work_area_id)
                continue
                @else
